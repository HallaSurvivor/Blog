...
_algebra_. Well, what happens if you have a surface $S$ and you compute 
...
the differential, but trust that it's really not _that_ hard[^3]!

I'm also not _really_ going to tell you how to compose two intersection 
points. But the rough idea is that you sum over all triangles containing
the two intersection points you have.
...
obvious why you might care about it. The obvious answer is 
[mirror symmetry][11], but I want to give a slightly anachronistic reason 
which I think is fairly down-to-earth.
...
The <span class=defn>Hall Algebra</span> of a category $\mathcal{C}$ has 
the (isomorphism classes of) objects of $\mathcal{C}$ as a basis. The 
product of objects $X$ and $Y$ is, roughly, the sum over all extensions $E$ 
fitting into an exact sequence $0 \to X \to E \to Y \to 0$.

For example, in case $\mathcal{C}$ is the category of abelian $p$-groups,
